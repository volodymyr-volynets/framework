...
class Debug
{
    ...
    public static function sendErrorsToAdmin()
    {
        ...
        $found = false;
        if (Application::get('debug.send_all_errors') && count(Base::$errors) > 0) {
            $found = true;
        } else {
    ...
    public static function render()
    {
        ...
        foreach (self::$data as $k => $v) {
            if ($k == 'errors') {
                $count = count(Base::$errors);
            } elseif ($k == 'classes') {
                $count = count($loaded_classes);
            } elseif ($k == 'application') {
        ...
        $result .= '<td>';
        $result .= '<h3>Errors (' . count(Base::$errors) . ')</h3>';
        $result .= '<table border="1" cellpadding="2" cellspacing="2" width="100%">';
        foreach (Base::$errors as $k => $v) {
            $result .= '<tr>';
            $result .= '<td><b>' . Base::$error_codes[$v['errno']] . ' (' . $v['errno'] . ') - ' . implode('<br/>', $v['error']) . '</b></td>';
            $result .= '</tr>';
            $result .= '<tr>';
