...
abstract class Collection
{
     ...
     * Main screen
     */
    public const MAIN_SCREEN = 'main';

    /**
     * Rows
     */
    public const ROWS = '__collection_rows';

    /**
     * Forms
     */
    public const FORMS = '__collection_forms';

    /**
     * Header row
     */
    public const HEADER_ROW = '__collection_header_row';

    /**
     * Main row
     */
    public const MAIN_ROW = '__collection_main_row';

    /**
     * Search row
     */
    public const SEARCH_ROW = '__collection_search_row';

    /**
     * Widgets row
     */
    public const FEATURED_ROW = '__collection_featured_row';

    /**
     * Widgets row
     */
    public const WIDGETS_ROW = '__collection_widgets_row';

    /**
     * Sub forms row
     */
    public const SUBFORMS_ROW = '__collection_subforms_row';
    ...
    public function render()
    {
            ...
            if (isset($this->options['forms'][$this->collection_screen_link][$form_link])) {
                ...
                if (isset($this->options['forms'][$this->collection_screen_link][$form_link]['bypass_input'])) {
                    foreach ($this->options['forms'][$this->collection_screen_link][$form_link]['bypass_input'] as $v0) {
                        ...
                        } elseif (isset($model_options['input'][$v0])) {
        ...
        foreach ($rows as $row_k => $row_v) {
            ...
            if (($row_v['options']['type'] ?? 'forms') == 'forms') {
            ...
            } elseif ($row_v['options']['type'] == 'tabs') { // tabs
                ...
                foreach ($forms as $form_k => $form_v) {
                    ...
                    if ((!empty($form_v['submodule']) && !\Can::submoduleExists($form_v['submodule'])) || empty($form_v['model'])) {
                        continue;
                    }
                ...
                if ($have_tabs) {
                    ...
                    $result[$index]['html'] = \HTML::message([
                        'type' => WARNING,
                        'options' => [
                            i18n(null, Messages::NO_PERMISSIONS_FOUND),
                        ]
                    ]);
                }
            }
