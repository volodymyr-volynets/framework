...
trait Trait2
{
     ...
     * @return Assembler
     */
    public static function queryAssemblerStatic(array & $data, array $options = []): Assembler
    {
        $class = get_called_class();
        $model = new $class();
    ...
    public function queryBuilder(array $options = []): Builder
    {
        ...
        if (empty($options['skip_acl'])) {
            $abac_class = Resources::getStatic('abac', 'model', 'get');
            if (!empty($abac_class)) {
    ...
    public function get($options = [])
    {
        ...
        } elseif (!empty($options['single_row'])) {
            $query->limit(1);
        }
        // memory caching
        ...
        if (!empty($options['cast_to_class']) && count($result['rows']) > 0) {
            foreach ($result['rows'] as $k => $v) {
                $destination = new $options['cast_to_class'](['skip_table_object' => true]);
                object_cast($destination, (object) $v);
                // we load full PK
                foreach ($this->pk as $v2) {
                    $destination->full_pk[$v2] = $v[$v2];
                }
     ...
     * @see $this->get()
     */
    public static function existsStatic(array $options = []): Builder
    {
        $class = get_called_class();
        $object = new $class();
     ...
     * @return Assembler
     */
    public function queryAssembler(array & $data, array $options = []): Assembler
    {
        $options['alias'] = $options['alias'] ?? 'relation_a';
        $object = new Assembler($this, $data, $options);
        return $object;
    }
}
