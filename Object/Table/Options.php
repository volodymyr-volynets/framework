...
class Options extends Data
{
    ...
    public function options($options = [])
    {
        ...
        if (!empty($options['__options_active'])) {
            ...
            $data = Common::filterActiveOptions($data, $options_active, $options['existing_values'] ?? [], $options['skip_values'] ?? []);
        } else {
            $data = Common::filterActiveOptions($data, false, $options['existing_values'] ?? [], $options['skip_values'] ?? []);
        }
    ...
    /**
     * Options active (static)
     *
     * @see $this::get()
     */
    public static function optionsActiveStatic(array $options = [])
    {
        $class = get_called_class();
        $object = new $class();
        $options['__options_active'] = true;
    ...
    public function optgroups($options = [])
    {
        if (empty($this->optgroups_map)) {
            ...
            return Common::optgroups($this->options($options), $this->optgroups_map, ['name' => 'name']);
        }
    }
    ...
    public function optionsGrouped($options = [])
    {
        ...
        if (!empty($result)) {
            $converted = Tree::convertByParent($result, 'parent_id');
            $result = [];
    ...
    public function presets($options = [])
    {
        ...
        } elseif (!is_array($options['columns'])) {
        ...
        // convert to tree
        $values_found = Tree::convertByParent($values_found, '__parent');
        $result = [];
        Tree::convertTreeToOptionsMulti($values_found, 0, ['name_field' => 'name'], $result);
        return $result;
    }
    ...
    public static function processOptionsExistingValuesAndSkipValues($value, $existing_values, $skip_values): bool
    {
        ...
        if (!empty($skip_values) && in_array($value, $skip_values)) {
            return false;
        }
        return true;
    }
