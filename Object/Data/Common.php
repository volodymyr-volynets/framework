...
class Common
{
    ...
    public static function processDomainsAndTypes($columns, $types = null, $model = null)
    {
        ...
        foreach ($columns as $k => $v) {
            if (isset($v['domain'])) {
                // multiple columns override
                if (strpos($v['domain'], '[]') !== false) {
                    $columns[$k]['domain'] = $v['domain'] = str_replace('[]', '', $v['domain']);
                    $columns[$k]['multiple_column'] = $v['multiple_column'] = true;
                }
            ...
            // multiple columns override
            if (isset($columns[$k]['type']) && strpos($columns[$k]['type'], '[]') !== false) {
                $columns[$k]['type'] = $v['type'] = str_replace('[]', '', $v['type']);
                $columns[$k]['multiple_column'] = $v['multiple_column'] = true;
            }
    ...
    public static function processOptions($model_and_method, $existing_object = null, $where = [], $existing_values = [], $skip_values = [], $options = [])
    {
        ...
        if (isset(self::$cached_options[$hash])) {
            ...
            $is_enum = strpos($model, '\Enum\\') !== false;
            if ($model == 'parent' && !empty($existing_object)) {
                $object = $existing_object->form_parent;
            } elseif ($model == 'this' && !empty($existing_object)) {
                $object = $existing_object;
            } elseif (!$is_enum) {
                $object = \Factory::model($model, true);
            }
            // for enum we do static call
            if ($is_enum) {
                $list = call_user_func([$model, $method]);
            } else {
    ...
    public static function options($data, $options_map, $options = [])
    {
        ...
        // inactive & icon_class
        $i18n_inactive = !empty($options['i18n']) ? i18n(null, Messages::INFO_INACTIVE) : Messages::INFO_INACTIVE;
        // mandatory column
        ...
        if (in_array('photo_id', $options_map_new)) {
            $generate_photo = Resources::getStatic('generate_photo');
    ...
    public static function buildOptions($data, $options_map, $orderby, $options)
    {
        ...
        $data = Common::options($data, $options_map, $options);
        // sorting
